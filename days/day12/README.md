...
That `Include` line is at the **top** of the file on Rocky 9, and sshd keeps the **first** value it reads for most keywords. So a drop-in beats the main file - the opposite of how Apache or nginx behave. Drop-ins also use lexical order, which is why the lab policy is named `00-lab-hardening.conf`: it must be read before Rocky and cloud-init’s `50-*` files. The later-file trap is the subject of failure 1.
