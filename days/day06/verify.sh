...
# "10.10.0.2" alone would also match 10.10.0.20 or 10.10.0.2/16 - and a /16
# here is failure 5 of break-and-fix.sh, which must not read as a pass.
vl_check "the client has an address on 10.10.0.0/24" 'ip netns exec client ip -br addr | grep -q "10.10.0.2/24"'
...
vl_manual "you saw a ping fail in one direction only, and proved which one"
